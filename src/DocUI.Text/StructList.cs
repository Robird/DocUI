...
/// <summary>
/// ⚠️ Never copy or assign <see cref="StructList{T}"/> by value; store it as a field and always pass/return by <c>ref</c> or <c>in</c> to avoid corrupting the shared backing array.
/// 值类型列表，通过 <c>ref</c> 返回避免元素复制，并可注入外部数组配合 <see cref="System.Buffers.ArrayPool{T}"/> 池化。
/// </summary>
/// <remarks>
/// ⚠️ 非线程安全：该类型不支持并发读写，必须由调用方在单一所有者或外部同步下使用。
/// ⚠️ 不可复制/克隆：此 <c>struct</c> 任何形式的按值复制（局部变量、赋值、隐式装箱）都会让多个实例共享并破坏同一后备数组。
/// ⚠️ 误用（例如在方法之间按值传递或从属性返回副本）会导致共享数组被竞争写入，从而损坏列表内容。
/// <para>Canonical usage guidelines:</para>
/// <list type="bullet">
/// <item>
/// <description>Store each <see cref="StructList{T}"/> as a private field or embedded member; avoid <c>var copy = list;</c> locals or assignments that duplicate the struct.</description>
/// </item>
/// <item>
/// <description>Pass between methods via <c>ref StructList&lt;T&gt;</c>, <c>in StructList&lt;T&gt;</c>, or <c>ref</c>/<c>ref readonly</c> returns; never pass or return by value.</description>
/// </item>
/// <item>
/// <description>Coordinate with <see cref="System.Buffers.ArrayPool{T}"/> by using the array-based constructors plus <see cref="Reset"/>/<see cref="Detach"/> to rent and return buffers deterministically.</description>
/// </item>
/// <item>
/// <description>Call <c>Clear()</c> or <c>Reset()</c> once work is complete to release references, and ensure a single owner manages the lifetime.</description>
/// </item>
/// <item>
/// <description><see cref="StructList{T}"/> enumerators capture a version stamp and throw <see cref="InvalidOperationException"/> if the list is structurally modified during iteration (fail-fast semantics, matching <see cref="List{T}"/>).</description>
/// </item>
/// </list>
...
internal struct StructList<T> {
    ...
    /// <summary>
    /// ⚠️ Never copy/assign <see cref="StructList{T}"/>; store it as a field and pass by <c>ref</c> to avoid corrupting the shared backing array。使用指定初始容量创建。
    /// </summary>
    /// <remarks>
    /// ⚠️ 始终以字段形式持有并通过 <c>ref</c>/<c>in</c> 传递；按值复制会让多个实例写入同一数组并破坏数据。
    /// 推荐的标准构造方式。若配合 ArrayPool 使用，请改用 <see cref="StructList(T[])"/>。
    /// </remarks>
    ...
    /// <summary>
    /// ⚠️ Never copy/assign <see cref="StructList{T}"/>; store it as state and only pass by <c>ref</c> or <c>in</c> to avoid corrupting the shared backing array。使用外部提供的数组创建（用于池化场景）。
    /// </summary>
    /// <remarks>
    /// ⚠️ 按值复制该实例会导致多个浅拷贝共享传入的 <paramref name="backingArray"/> 并互相破坏内容。
    ...
    /// <summary>
    /// ⚠️ Never copy/assign <see cref="StructList{T}"/>; surface it via <c>ref</c> to avoid corrupting the shared backing array。使用外部数组并设置初始有效元素数（用于包装已有数据）。
    /// </summary>
    /// <remarks>
    /// ⚠️ 复制该实例会让多个包装器共享 <paramref name="backingArray"/>，从而在写入时互相覆盖。
    ...
    private void SetCapacity(int capacity) {
        ...
        int newCount = Math.Min(_count, capacity);
        if (newCount > 0 && _items is not null) {
            Array.Copy(_items, newItems, newCount);
        }
        _items = newItems;
        _count = newCount;
        IncrementVersion();
    ...
    public Enumerator GetEnumerator() => new(ref Unsafe.AsRef(in this));

    /// <summary>零分配枚举器。</summary>
    public ref struct Enumerator {
        private readonly ref StructList<T> _list;
        private readonly T[] _items;
        private readonly int _count;
        private readonly int _version;
        private int _index;

        internal Enumerator(ref StructList<T> list) {
            _list = ref list;
        ...
        public readonly ref T Current {
            get {
                ...
                if ((uint)_index >= (uint)_count)
                    ThrowEnumerationInvalid();
                return ref _items[_index];
            }
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private readonly void EnsureVersion() {
            if (_version != _list._version)
                ThrowEnumeratorVersionChanged();
        }

        [DoesNotReturn]
        [MethodImpl(MethodImplOptions.NoInlining)]
        private static void ThrowEnumeratorVersionChanged() =>
            throw new InvalidOperationException("StructList was modified during enumeration.");

        [DoesNotReturn]
        [MethodImpl(MethodImplOptions.NoInlining)]
        private static void ThrowEnumerationInvalid() =>
